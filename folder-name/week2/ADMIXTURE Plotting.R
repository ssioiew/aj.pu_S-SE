...
# Paths
q_dir <- "week2/aligned_results"
fam_path <- "week2/modern_ancient_no_maniri.fam"
metadata_path <- "SK_PL_31-july-E_SE_no_redundance.xlsx"

# Paths for missingness data
missingness_file_path <- "week2/missingness_report.imiss"
...
create_ggplot <- function(k) {
  q_file <- file.path(q_dir, sprintf("modern_ancient_no_maniri.%d.Q", k))
...
for (k in k_new) {
  q_file <- file.path(q_dir, sprintf("modern_ancient_no_maniri.%d.Q", k_new))
  q_data <- read.table(q_file, header = FALSE)
  mat <- t(as.matrix(q_data))
